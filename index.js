...
app.get('/genre*', (req, res) => {
  ...
  let searchString = req.query['q'].replace(/\s/g, '%20');
    ...
    axios.get(`https://api.spotify.com/v1/search?q=genre:%22${searchString}%22&type=artist`, options)
      .then ( (subRes) => {
        //console.log("back from spotify(SERVER): ", subRes.data.artists.items);
        res.send(subRes.data.artists.items);
      })
...
app.get('/track*', (req, res) => {
  ...
  let searchString = req.query['q'].replace(/\s/g, '%20');
    ...
    axios.get(`https://api.spotify.com/v1/search?q=${searchString}&type=track`, options)
      .then ( (subRes) => {
        //console.log("back from spotify(SERVER): ", subRes.data.artists.items);
...
app.get('/meta*', (req, res) => {
  ...
  console.log("the query: ", req.query);
    ...
    axios.get(`https://api.spotify.com/v1/audio-features/${req.query.q}`, options)
      .then ( (subRes) => {
        //console.log("back from spotify(SERVER): ", subRes.data.artists.items);
        res.send(subRes.data);
      })
